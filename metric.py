...
def levenshtein(pred, gt, norm=True):
    ...
    if norm:
        score = (1 - dp[n][m] / (max(n, m)+ 1e-7)) * 100
    else:
        score = dp[n][m]
...
class ScoreMeter(object):
    ...
    def update(self, pred, gt):
        ...
        # print("p_label",p_label, "g_label",g_label)
        if len(g_label) != 0:
    ...
    def get_scores(self):
        ...
        for i in range(len(self.thresholds)):
            precision = self.tp[i] / float(self.tp[i] + self.fp[i]+ 1e-7)
            recall = self.tp[i] / float(self.tp[i] + self.fn[i]+ 1e-7)
...
def remove_repeat_label(label):
    video_len = len(label)
    new_label = [label[0]]
    for i in range(video_len-1):
        if label[i+1] != new_label[-1]:
